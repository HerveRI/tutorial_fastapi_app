import pytest
from app import schema, db_models


@pytest.fixture()
def test_vote(test_posts, session, test_user):
    new_vote = db_models.Vote(post_id = test_posts[3].id, user_id = test_user['id'])
...
def test_vote_post_non_exist(authorized_client, test_posts):
    res = authorized_client.post("/vote", json={"post_id": 290982, "dir": 0})
    assert res.status_code == 404

def test_vote_unauthorizer_user(client, test_posts):
